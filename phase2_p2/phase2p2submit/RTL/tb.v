...
module tb();
`define IPNUT_FILE_NAME "ddr2_test_pattern4_block.dat"
`define TRACE_FILE_NAME "ddr2_test.trc"  //Not used 
`define DUMP_FILE_NAME  "ddr2_out_blk.dump"
...
`define NULL 0  
   ...
   // define clocks
   initial clk = 0;
   always #1.3 clk = ~clk; // 380MHz
   
//initial
//$sdf_annotate("../sdf/ddr2_controller.sdf",XCON,,,"TYPICAL","1.0:1.0:1.0", "FROM_MTM");
   ...
   always @ (ApplyTestPattern)
	 begin
		if (BlkWriteInProgress)
		  begin
			 cmd      <= #0.1 3'bx;////////////////need to change this back to x later. mc
			 din      <= #0.1 Data;
			 addr     <= #0.1 25'bx;
			 sz       <= #0.1 2'bx;
			 op       <= #0.1 3'bx; 
   ...
   ddr2_controller XCON (
						 ...
						 //.dqsbar_pad			(c0_dqsbar_pad[1:0]),//comment out to resolve differential dqs disabled error
						 // Inputs
						 .clk					(clk),
						 .reset					(reset),
						 .cmd					(cmd[2:0]),
						 .sz					(sz[1:0]), // not used in part1
						 .op					(op[2:0]),
						 .din					(din[15:0]),
						 .addr					(addr[24:0]),
						 .read				(fetching),
